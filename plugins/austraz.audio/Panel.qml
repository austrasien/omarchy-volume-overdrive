...
Panel {
  ...
  function setOutputVolume(v) {
    ...
    var volume = Math.max(0, Math.min(2, v))
        ...
        Column {
          ...
          Column {
            ...
            CursorSurface {
              ...
              readonly property real outputLive: outputSlider.dragging ? outputSlider.liveValue : root.outputVolume
              readonly property bool outputOverdriven: outputLive > 1
              ...
              PanelSlider {
                ...
                fillColor: outputSliderRow.outputOverdriven ? Color.urgent : (root.bar ? root.bar.foreground : Color.foreground)
                knobColor: fillColor
                opacity: root.outputMuted ? 0.5 : 1.0
                enabled: !!root.sink
              ...
              // Notch at real 100% on the 0–200% output scale.
              Rectangle {
                width: Math.max(1, Style.space(2))
                height: outputSlider.trackHeight + Style.space(4)
                radius: 1
                color: root.bar ? root.bar.foreground : Color.foreground
                opacity: 0.85
                anchors.verticalCenter: parent.verticalCenter
                x: outputSlider.x + Math.round(outputSlider.width * 0.5 - width / 2)
                z: 2
              }
