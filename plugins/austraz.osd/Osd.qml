...
Item {
  ...
  // digits don't jitter between 9% and 200%.
  ...
  // Overdrive scale (volume up to 200%): mark the real 100% point and turn the
  // fill urgent past it. Brightness/other OSDs keep max 100 and skip both.
  readonly property bool overdriveScale: root.hasProgress && root.maxValue > 100
  readonly property bool overdriven: root.overdriveScale && root.value > 100
  readonly property real normMark: root.overdriveScale ? (100 / root.maxValue) : 0
    ...
    BorderSurface {
      ...
      Row {
        ...
        Item {
          ...
          width: root.barWidth
          height: Math.max(Style.space(10), Style.spacing.sm + Style.space(4))
          anchors.verticalCenter: parent.verticalCenter
          ...
          Rectangle {
            ...
            Rectangle {
              ...
              color: root.overdriven ? Color.urgent : Color.accent
          ...
          // Fixed notch at real 100% when the bar scale goes past NORM.
          Rectangle {
            visible: root.overdriveScale
            width: Math.max(1, Style.space(2))
            height: track.height + Style.space(4)
            radius: 1
            color: Color.popups.text
            anchors.verticalCenter: track.verticalCenter
            x: Math.round(track.width * root.normMark - width / 2)
            z: 2
          }
        }
        Text {
